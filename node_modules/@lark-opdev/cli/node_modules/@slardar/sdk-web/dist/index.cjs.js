...
var applyMonitor = function (monitorCtor, props, cb, deps) {
    ...
    if (deps === void 0) { deps = []; }
    try {
        var monitor = monitorCtor.apply(void 0, tslib.__spreadArray([], tslib.__read(deps), false));
...
var onPageUnload = function (cb) {
    ...
    if (!window)
        return;
    var _a = tslib.__read(invokeCallbackOnce(cb), 1), invokeCbOnce = _a[0];
...
var getConfig = function (c, defaultConfig) {
    if (isObject(c)) {
        return tslib.__assign(tslib.__assign({}, defaultConfig), c);
    }
    else {
...
var hookOpen = function (open) {
    return function () {
        ...
        _a = tslib.__read(openOptions, 2), this._method = _a[0], this._url = _a[1];
...
var hookSetHeader = function (setRequestHeader) {
    return function () {
        ...
        var _a = tslib.__read(setOptions, 2), name = _a[0], value = _a[1];
...
var observePerf = function (Ob, pipe, types) {
    var _a = tslib.__read(applyPerformanceObserver(Ob, pipe), 3), observe = _a[0], disconnect = _a[2];
    observe(types);
    return disconnect;
};
var observePerfWithBuffer = function (Ob, pipe, type) {
    var _a = tslib.__read(applyPerformanceObserver(Ob, pipe), 3), bufferedObserver = _a[1], disconnect = _a[2];
...
var httpGetterWithXhrObserver = function (report, tearDownGroup, _a, xhrConfig) {
    var _b = tslib.__read(_a, 2), xhrObserver = _b[0], getResourceObserver = _b[1];
    ...
    tearDownGroup.push(xhrObserver[0](function (_a) {
        var _b = tslib.__read(_a, 4); _b[0]; var _url = _b[1]; _b[2]; var xhr = _b[3];
...
function formatXHRAllResponseHeaders(headers) {
    if (isString(headers) && headers) {
        return headers.split('\r\n').reduce(function (result, line) {
            if (isString(line)) {
                var _a = tslib.__read(line.split(': '), 2), name_1 = _a[0], value = _a[1];
...
var reportOnInitCommonParams = function (client, overrides) {
    ...
    return function (ev) {
        client.report(tslib.__assign(tslib.__assign({}, ev), { overrides: tslib.__assign(tslib.__assign({}, common), ((overrides && overrides(ev)) || {})) }));
    };
};
...
var getCbHook = function (client) { return function (cb) {
    ...
    return function (ev) {
        cb(tslib.__assign(tslib.__assign({}, ev), { overrides: tslib.__assign(tslib.__assign({}, common), { timestamp: ev.payload.request.timestamp }) }));
    };
}; };
function AjaxMonitorPlugin(client) {
    client.on('init', function () {
        ...
        var tearDownGroup = [];
        var xhrConfig = tslib.__assign(tslib.__assign({}, config), { hookCbAtReq: getCbHook(client), setTraceHeader: setTraceContext(config.trace) });
...
function AjaxMonitorVolPlugin(client) {
    client.on('init', function () {
        ...
        var tearDownGroup = [];
        var xhrConfig = tslib.__assign(tslib.__assign({}, config), { hookCbAtReq: getCbHook(client), setTraceHeader: setVolTraceContext(config.trace, "app_id=" + ((_a = client.config()) === null || _a === void 0 ? void 0 : _a.aid) + ",origin=web") });
        var getResourceSubject = function () { return initSubjectInGlobal(client, resourceSubject); };
        xhrConfig.autoWrap &&
            httpGetterWithXhrObserver(client.report.bind(client), tearDownGroup, [
                initSubjectInGlobal(client, [XHR_SUBJECT_NAME, applyXHR(XMLHttpRequest && XMLHttpRequest.prototype)]),
...
var applyBreadcrumb = function (maxBreadcrumbs, onAddBreadcrumb, onMaxBreadcrumbs) {
    ...
    var addBreadcrumb = function (breadcrumb) {
        ...
        if (processed) {
            var mergedBreadcrumb = tslib.__assign(tslib.__assign({}, breadcrumb), { timestamp: breadcrumb.timestamp || now() });
            breadcrumbs =
                maxBreadcrumbs >= 0 && breadcrumbs.length + 1 > maxBreadcrumbs
                    ? onMaxBreadcrumbs(tslib.__spreadArray(tslib.__spreadArray([], tslib.__read(breadcrumbs), false), [mergedBreadcrumb], false), maxBreadcrumbs)
                    : tslib.__spreadArray(tslib.__spreadArray([], tslib.__read(breadcrumbs), false), [mergedBreadcrumb], false);
        }
    };
...
var BreadcrumbMonitor = function (document) {
    ...
    return function (props, _cb) {
        ...
        var _a = tslib.__read(applyDomAndKeyPress(100), 2), domEventHandler = _a[0], keypressHandler = _a[1];
        var _b = tslib.__read(applyBreadcrumb(maxBreadcrumbs, onAddBreadcrumb, onMaxBreadcrumbs), 2), getBreadcrumbs = _b[0], addBreadcrumb = _b[1];
...
function BreadcrumbMonitorPlugin(client) {
    client.on('init', function () {
        ...
        var _a = tslib.__read(applyMonitor(BreadcrumbMonitor, config, noop), 3), getBreadcrumbs = _a[0], addBreadcrumb = _a[1], tearDown = _a[2];
...
var httpGetterWithFetchObserver = function (report, tearDownGroup, _a, fetchConfig) {
    var _b = tslib.__read(_a, 2), fetchObserver = _b[0], getResourceObserver = _b[1];
    ...
    tearDownGroup.push(fetchObserver[0](function (_a) {
        var _b = tslib.__read(_a, 2), req = _b[0], options = _b[1];
...
var isHttpURL = function (url) {
    ...
    var _a = tslib.__read(url.split(':'), 2), protocol = _a[0], path = _a[1];
...
var addHeader = function (key, value, req, options, Request, Headers) {
    ...
    if (isRequest(req, Request)) {
        ...
        options.headers = tslib.__assign(tslib.__assign({}, options.headers), (_a = {}, _a[key] = value, _a));
    }
};
...
function FetchMonitorPlugin(client) {
    client.on('init', function () {
        ...
        var tearDownGroup = [];
        var fetchConfig = tslib.__assign(tslib.__assign({}, config), { hookCbAtReq: getCbHook(client), setTraceHeader: setTraceContext(config.trace) });
...
function FetchMonitorVolPlugin(client) {
    client.on('init', function () {
        ...
        var tearDownGroup = [];
        var fetchConfig = tslib.__assign(tslib.__assign({}, config), { hookCbAtReq: getCbHook(client), setTraceHeader: setVolTraceContext(config.trace, "app_id=" + ((_a = client.config()) === null || _a === void 0 ? void 0 : _a.aid) + ",origin=web") });
        fetchConfig.autoWrap &&
...
var normalizeException = function (event) {
    ...
    try {
        ...
        if (error) {
            ...
            // eslint-disable-next-line @typescript-eslint/prefer-optional-chain
            return tslib.__assign(tslib.__assign({}, rejectionError), { name: (_a = (rejectionError && rejectionError.name)) !== null && _a !== void 0 ? _a : 'UnhandledRejection' });
        }
    }
    catch (_oO) {
        // no-empty
...
var hookGlobalAsync = function (report, window, xhr) {
    ...
    var wrap = function (fn, sourceData) {
        ...
        var source = {
            ...
            data: tslib.__assign({}, sourceData),
        };
    ...
    window && restoreFns.push.apply(restoreFns, tslib.__spreadArray([], tslib.__read(DEFAULT_TIME_FUNCTION.filter(function (timeFunc) { return window[timeFunc]; }).map(function (timeFunc) {
        return hookObjectProperty(window, timeFunc, function (origin) {
            return function (handler) {
                ...
                return (origin && origin.call.apply(origin, tslib.__spreadArray([this,
                    wrap(handler, {
                        function: timeFunc,
                    })], tslib.__read(params), false)));
            };
        }, false)();
    })), false));
    xhr &&
...
var jsErrorGetterWithContext = function (report, tearDownGroup, _a, config) {
    var _b = tslib.__read(_a, 2), errorObserver = _b[0], promiseRejectionObserver = _b[1];
    ...
    captureGlobalAsync && tearDownGroup.push.apply(tearDownGroup, tslib.__spreadArray([], tslib.__read(hookGlobalAsync(reportJsError)), false));
...
var pvGetterWithRouteObserver = function (report, tearDownGroup, routeObservers, config) {
    ...
    var _a = tslib.__read(applyOnPidChange(applyReportPageview(report), initPid || getFinalPid(location.href), extractPidFromUrl(location.href), onPidUpdate), 2), onPidChange = _a[0], sendInit = _a[1];
    if (!isManualMode(routeMode)) {
        var _b = tslib.__read(applyOnUrlChange(function (sources, url) { return onPidChange(sources, extractPidFromUrl(url), customPidFromUrl(url)); }, ''), 1), onUrlChange_1 = _b[0];
...
function PageviewMonitorPlugin(client) {
    client.on('init', function () {
        ...
        var _b = tslib.__read(pvGetterWithRouteObserver(client.report.bind(client), tearDownGroup, isManualMode(routeMode) ? [] : [client.initSubject(hashSubject), client.initSubject(historySubject)], tslib.__assign(tslib.__assign({}, config), { initPid: (_a = client.config()) === null || _a === void 0 ? void 0 : _a.pid, onPidUpdate: function (pid) {
...
var observeUnload = function (next, tearDown) {
    ...
    if (!window)
        return;
    // eslint-disable-next-line @typescript-eslint/no-invalid-void-type
    var _a = tslib.__read(invokeCallbackOnce(next), 1), invokeCbOnce = _a[0];
...
var resourceGetterWithContext = function (report, tearDownGroup, _a, config) {
    var _b = tslib.__read(_a, 2), loadObserver = _b[0], getResourceObserver = _b[1];
    var performance = getDefaultPerformance();
    if (!performance)
        return;
    ...
    tearDownGroup.push(loadObserver[0](function () {
        var _a = tslib.__read(applyPerformance(performance), 3), timing = _a[0], getEntriesByType = _a[2];
...
var resourceErrorGetterWithContext = function (report, tearDownGroup, _a, config) {
    var _b = tslib.__read(_a, 1), errorObserver = _b[0];
    var window = getDefaultBrowser();
    if (!window)
        return;
    ...
    var ignoreRegExp = getRegexp(ignoreUrls);
    var _c = tslib.__read(applyPerformance(getDefaultPerformance()), 5), getEntriesByName = _c[4];
...
var initMetric = function (name, value) {
    return tslib.__assign({ name: name, value: value }, defaultMetricContext);
};
...
var getPaint = function (entryName, metricName, report, tearDownGroup, _a) {
    var _b = tslib.__read(_a, 2), hiddenObserver = _b[0], unloadObserver = _b[1];
    var PerformanceObserver = getDefaultPerformanceObserver();
    var performance = getDefaultPerformance();
    ...
    var _c = tslib.__read(applyPerformance(performance), 5), getEntriesByName = _c[4];
...
var calcLastNet2Busy = function (undoneReqStarts, observedResReqs, now) {
    ...
    for (var i = endpoints.length - 1; i >= 0; i--) {
        var _a = tslib.__read(endpoints[i], 2), timestamp = _a[0], type = _a[1];
        switch (type) {
            case 0 /* S */:
                currentActive--;
                break;
...
var observeResFetchMutations = function (MutationObserver, callback) {
    ...
    var _a = tslib.__read(applyMutationObserver(MutationObserver, function (mutations) {
...
var getLastBusyAndLongTasks = function (tearDownGroup, _a, entries) {
    var _b = tslib.__read(_a, 5), xhrObserver = _b[0], fetchObserver = _b[1], longtaskObserver = _b[2], resourceObserver = _b[3], MutationObserver = _b[4];
    return function (reschedule, now$1) {
        var longTasks = [];
        var networkRequests = [];
        var _a = tslib.__read(applyRecord(), 3), incompleteInitReqStartTimes = _a[0], before = _a[1], after = _a[2];
        ...
        var uniqId = 0;
        tearDownGroup.push(xhrObserver[0](function (_a) {
            var _b = tslib.__read(_a, 1), _method = _b[0];
        ...
        var _b = tslib.__read((MutationObserver && observeResFetchMutations(MutationObserver, function () { return reschedule(now$1() + QUIET_TIME_LIMIT); })) || [], 2), observeRes = _b[0], stopObserveRes = _b[1];
...
var getTTI = function (report, tearDownGroup, _a, config) {
    var _b = tslib.__read(_a, 5), xhrObserver = _b[0], fetchObserver = _b[1], longtaskObserver = _b[2], resourceObserver = _b[3], pvObserver = _b[4];
    ...
    var _e = tslib.__read(applyPerformance(performance), 5), timing = _e[0], now = _e[1], getEntriesByName = _e[4];
    var _f = tslib.__read(applyScheduler(now), 3), startSchedule = _f[0], stopSchedule = _f[1], reschedule = _f[2];
    var _g = tslib.__read(getLastBusyAndLongTasks(tearDownGroup, [xhrObserver, fetchObserver, longtaskObserver, resourceObserver, getDefaultMutationObserver()], entries)(reschedule, now), 2), longTasks = _g[0], getLastBusy = _g[1];
...
var getFMPInternal = function (_a) {
    var _b = _a === void 0 ? [] : _a, _c = tslib.__read(_b), first = _c[0], rest = _c.slice(1);
    return ((rest &&
        rest.reduce(function (_a, cur) {
            var _b = tslib.__read(_a, 2), prev = _b[0], target = _b[1];
...
var getFMP = function (report, tearDownGroup, _a) {
    var _b = tslib.__read(_a, 1), loadObserver = _b[0];
    var document = getDefaultDocument();
    ...
    var _c = tslib.__read(applyAnimationFrame(document, raf, caf, true), 1), scheduleAF = _c[0];
    // 持续监听 dom 变化，记录下时间和得分
    // scheduleAF 可以避免强制回流，同时真正的变更是在animationFrame 中
    var _d = tslib.__read(applyMutationObserver(MutationObserver, function () { return scheduleAF(record); }), 2), observe = _d[0], disconnect = _d[1];
...
var BlankScreenMonitor = function (win, document, performanceObserver, mutationObserver, performance) {
    ...
    return function (_b, cb) {
        ...
        var _c = tslib.__read(applyPerformance(performance), 2), relativeNow = _c[1];
        var blankInfo;
        var errorInfo;
        ...
        var _d = tslib.__read(applyMutationObserver(mutationObserver, schedule), 2), mutObserve = _d[0], mutDisconnect = _d[1];
        var _e = tslib.__read(applyPerformanceObserver$1(performanceObserver, function (_a, _i, arr) { return timer && arr.length > 1 && schedule(); }), 2), perfObserve = _e[0], perfDisconnect = _e[1];
        if (autoDetect)
...
function BlankScreenMonitorBasePlugin(client, defaultConfig, overrideConfig) {
    client.on('init', function () {
        ...
        var _a = tslib.__read(applyMonitor(BlankScreenMonitor, config, client.report.bind(client)), 3), tearDown = _a[0], applyOtherReport = _a[1], detectBlankScreen = _a[2];
...
var applyIncompleteReq = function (tearDownGroup, _a, notify) {
    var _b = tslib.__read(_a, 2), xhrObserver = _b[0], fetchObserver = _b[1];
    var _c = tslib.__read(applyRecord(), 3), incompleteReq = _c[0], before = _c[1], after = _c[2];
    var uniqId = 0;
    tearDownGroup.push(xhrObserver[0](function (_a) {
        var _b = tslib.__read(_a, 1); _b[0];
        var requestId = (uniqId += 1);
        before(requestId, now());
        notify();
        return function () {
...
var actionGetterWithFetchObserver = function (report, tearDownGroup, _a, config) {
    ...
    var _c = tslib.__read(applyPerformance(performance), 2), now = _c[1];
    // 结算
    var reportActionIfAllowed = function () {
        ...
        // request duration
        var requestDuration = requestStartAndEnd.length > 0 ? Math.max.apply(Math, tslib.__spreadArray([], tslib.__read(requestStartAndEnd), false)) - Math.min.apply(Math, tslib.__spreadArray([], tslib.__read(requestStartAndEnd), false)) : 0;
        ...
        report({
            ...
            payload: tslib.__assign(tslib.__assign({}, currentPayload), { duration: actionDuration, frontend_time: actionDuration - requestDuration }),
        });
        // 上报后重置数据
        resetAction();
    };
    ...
    var _d = tslib.__read(waitActionComplete(reportActionIfAllowed, resetAction, window), 3), start = _d[0], notify = _d[1], complete = _d[2];
    var incompleteReq = applyIncompleteReq(tearDownGroup, [xhrObserver, fetchObserver], notify);
    var _e = tslib.__read(applyMutationObserver(MutationObserver, notify), 2), observe = _e[0], disconnect = _e[1];
    observe();
    tearDownGroup.push(disconnect);
    // 补充数据
    var applyOtherReport = function (ev) {
    ...
    var initAction = function (target, type) {
        ...
        var actionId = uuid();
        var _a = tslib.__read(applyPerformance(performance), 2), now = _a[1];
        // init payload
        currentPayload = {
            start_time: now(),
            id: actionId,
...
function ActionMonitorBasePlugin(client, config) {
    ...
    if (!document || !window)
        return;
    var _a = tslib.__read(actionGetterWithFetchObserver(client.report.bind(client), tearDownGroup, [
        initSubjectInGlobal(client, [XHR_SUBJECT_NAME, applyXHR(XMLHttpRequest && XMLHttpRequest.prototype)]),
        initSubjectInGlobal(client, fetchSubject),
...
var getFID = function (report, tearDownGroup) {
    ...
    var _a = tslib.__read(applyPerformance(performance), 3), getEntriesByType = _a[2];
...
var getCLS = function (report, tearDownGroup, _a) {
    var _b = tslib.__read(_a, 3), hiddenObserver = _b[0], unloadObserver = _b[1], pvObserver = _b[2];
    var PerformanceObserver = getDefaultPerformanceObserver();
    ...
    var _c = tslib.__read(applyHandleEntries(), 2), resetSessionValue = _c[0], handleEntriesWithMetric = _c[1];
...
var getLongtask = function (report, tearDownGroup, _a) {
    var _b = tslib.__read(_a, 3), longtaskObserver = _b[2];
    tearDownGroup.push(longtaskObserver[0](function (entry) {
...
var getTiming = function (report, tearDownGroup, _a) {
    var _b = tslib.__read(_a, 2), loadObserver = _b[0], unloadObserver = _b[1];
    var performance = getDefaultPerformance();
    var _c = tslib.__read(applyPerformance(performance), 3), getEntriesByType = _c[2];
...
var getMPFID = function (report, tearDownGroup, _a) {
    var _b = tslib.__read(_a, 3), loadObserver = _b[0], longtaskObserver = _b[2];
    var PerformanceObserver = getDefaultPerformanceObserver();
    var performance = getDefaultPerformance();
    ...
    var calculateAndSendMetric = function () {
        var _a = tslib.__read(applyPerformance(performance), 5), getEntriesByName = _a[4];
    ...
    client.on('init', function () {
        ...
        var reportMetric = function (data) {
            ...
            var payloadWithOverrides = (data.ev_type === SINGLE_METRIC_EV_TYPE && data.payload.name === CLS[0]) ||
                ...
                ? data
                : tslib.__assign(tslib.__assign({}, data), { overrides: overrides });
        ...
        // spa_load
        var _b = tslib.__read(getSPA$1(client.report.bind(client)), 2), performanceInit = _b[0], performanceSend = _b[1];
        ...
        var sendPref = function (customMetric) {
            var payload = tslib.__assign(tslib.__assign(tslib.__assign({}, defaultMetricContext), customMetric), { isCustom: true });
...
var getSPA = function (report, tearDownGroup, _a) {
    var _b = tslib.__read(_a, 6), pvObserver = _b[0], xhrObserver = _b[1], fetchObserver = _b[2], resourceObserver = _b[3], longtaskObserver = _b[4], MutationObserver = _b[5];
    ...
    var _c = tslib.__read(waitActionComplete(reportSPAIfAllowed, endSPA, window), 3), start = _c[0], notify = _c[1], complete = _c[2];
    // request
    var incompleteReq = applyIncompleteReq(tearDownGroup, [xhrObserver, fetchObserver], notify);
    // resource
    ...
    // dom
    var _d = tslib.__read(applyMutationObserver(MutationObserver, notify), 2), observe = _d[0], disconnect = _d[1];
    tearDownGroup.push(disconnect);
    var isFirstPV = true;
...
var InjectNetworkTypePlugin = function (client) {
    ...
    client.on('report', function (ev) {
        return tslib.__assign(tslib.__assign({}, ev), { extra: tslib.__assign(tslib.__assign({}, (ev.extra || {})), { network_type: network_type }) });
    });
};
...
exports.ACTION_MONITOR_PLUGIN_NAME = ACTION_MONITOR_PLUGIN_NAME;
exports.AJAX_MONITOR_PLUGIN_NAME = AJAX_MONITOR_PLUGIN_NAME;
exports.ActionMonitorBasePlugin = ActionMonitorBasePlugin;
exports.ActionMonitorPlugin = ActionMonitorPlugin;
exports.AjaxMonitorPlugin = AjaxMonitorPlugin;
exports.AjaxMonitorVolPlugin = AjaxMonitorVolPlugin;
exports.BLANK_SCREEN_MONITOR_PLUGIN_NAME = BLANK_SCREEN_MONITOR_PLUGIN_NAME;
exports.BREADCRUMB_MONITOR_PLUGIN_NAME = BREADCRUMB_MONITOR_PLUGIN_NAME;
exports.BlankScreenMonitorPlugin = BlankScreenMonitorPlugin;
exports.BlankScreenMonitorVolPlugin = BlankScreenMonitorVolPlugin;
exports.BreadcrumbMonitorPlugin = BreadcrumbMonitorPlugin;
exports.CustomPlugin = CustomPlugin;
exports.FETCH_MONITOR_PLUGIN_NAME = FETCH_MONITOR_PLUGIN_NAME;
exports.FMPMonitorPlugin = FMPMonitorPlugin;
exports.FMP_MONITOR_PLUGIN_NAME = FMP_MONITOR_PLUGIN_NAME;
exports.FetchMonitorPlugin = FetchMonitorPlugin;
exports.FetchMonitorVolPlugin = FetchMonitorVolPlugin;
exports.HEATMAP_MONITOR_PLUGIN_NAME = HEATMAP_MONITOR_PLUGIN_NAME;
exports.InjectNetworkTypePlugin = InjectNetworkTypePlugin;
exports.JS_ERROR_EV_TYPE = JS_ERROR_EV_TYPE$1;
exports.JS_ERROR_MONITOR_PLUGIN_NAME = JS_ERROR_MONITOR_PLUGIN_NAME;
exports.JsErrorMonitorPlugin = JsErrorMonitorPlugin;
exports.PAGEVIEW_MONITOR_PLUGIN_NAME = PAGEVIEW_MONITOR_PLUGIN_NAME;
exports.PERFORMANCE_MONITOR_PLUGIN_NAME = PERFORMANCE_MONITOR_PLUGIN_NAME;
exports.PageviewMonitorPlugin = PageviewMonitorPlugin;
exports.PerformanceMonitorPlugin = PerformanceMonitorPlugin;
exports.RESOURCE_ERROR_MONITOR_PLUGIN_NAME = RESOURCE_ERROR_MONITOR_PLUGIN_NAME;
exports.RESOURCE_MONITOR_PLUGIN_NAME = RESOURCE_MONITOR_PLUGIN_NAME;
exports.ResourceErrorMonitorPlugin = ResourceErrorMonitorPlugin;
exports.ResourceMonitorPlugin = ResourceMonitorPlugin;
exports.SPALoadMonitorPlugin = SPALoadMonitorPlugin;
exports.SamplePlugin = SamplePlugin;
exports.TTIMonitorPlugin = TTIMonitorPlugin;
exports.TTI_MONITOR_PLUGIN_NAME = TTI_MONITOR_PLUGIN_NAME;
exports.UNKNOWN_PATH = UNKNOWN_PATH;
exports.applyAnimationFrame = applyAnimationFrame;
exports.applyDomAndKeyPress = applyDomAndKeyPress;
exports.applyMonitor = applyMonitor;
exports.applyMutationObserver = applyMutationObserver;
exports.applyPerformance = applyPerformance;
exports.applyPerformanceObserver = applyPerformanceObserver$1;
exports.applyRequestIdleCallback = applyRequestIdleCallback;
exports.applyUpdate = applyUpdate;
exports.captureCurrentContext = captureCurrentContext;
exports.createBatchSender = createBatchSender;
exports.createBrowserSender = createBrowserSender;
exports.defaultPerformancePrecollect = defaultPerformancePrecollect;
exports.domBreadcrumb = domBreadcrumb;
exports.getBeaconTransport = getBeaconTransport;
exports.getConfig = getConfig;
exports.getCookieId = getCookieId;
exports.getCurrentScript = getCurrentScript;
exports.getDefaultBrowser = getDefaultBrowser;
exports.getDefaultCaf = getDefaultCaf;
...
exports.getDefaultMutationObserver = getDefaultMutationObserver;
exports.getDefaultNavigator = getDefaultNavigator;
exports.getDefaultNetworkInformation = getDefaultNetworkInformation;
exports.getDefaultPerformance = getDefaultPerformance;
exports.getDefaultPerformanceObserver = getDefaultPerformanceObserver;
exports.getDefaultPerformanceTiming = getDefaultPerformanceTiming;
exports.getDefaultPromise = getDefaultPromise;
exports.getDefaultRaf = getDefaultRaf;
...
exports.getValueFromCookieName = getValueFromCookieName;
exports.getXhrTransport = getXhrTransport;
exports.htmlTreeAsString = htmlTreeAsString;
exports.invokeCallbackOnce = invokeCallbackOnce;
exports.loadScript = loadScript;
exports.normalizeCustomEventData = normalizeCustomEventData;
exports.normalizeCustomLogData = normalizeCustomLogData;
...
exports.onceHidden = onceHidden;
exports.parseUrl = parseUrl;
exports.removeStorageItem = removeStorageItem;
exports.reportOnInitCommonParams = reportOnInitCommonParams;
exports.reportSelfError = reportSelfError;
exports.setCookie = setCookie;
exports.setStorageItem = setStorageItem;
exports.syncReportWithCapturedContext = syncReportWithCapturedContext;
