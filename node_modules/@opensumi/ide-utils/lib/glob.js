...
exports.getPathTerms = exports.getBasenameTerms = exports.isRelativePattern = exports.hasSiblingFn = exports.hasSiblingPromiseFn = exports.parseGlob = exports.match = exports.splitGlobAware = exports.getEmptyExpression = void 0;
...
function wrapRelativePattern(parsedPattern, arg2) {
    ...
    return function (path, basename) {
        var _a;
        const base = new path_1.Path(arg2.base);
        const target = new path_1.Path(path);
        if (!base.isEqualOrParent(target)) {
            return null;
        }
        return parsedPattern(((_a = base.relative(target)) === null || _a === void 0 ? void 0 : _a.toString()) || '', basename);
    };
}
...
function trivia4and5(path, pattern, matchPathEnds) {
    const nativePath = path_1.Path.separator !== path_1.Path.nativeSeparator ? path.replace(ALL_FORWARD_SLASHES, path_1.Path.separator) : path;
    const nativePathEnd = path_1.Path.separator + nativePath;
...
function match(arg1, path, hasSibling) {
    ...
    return parseGlob(arg1)(path, undefined, hasSibling);
}
exports.match = match;
function parseGlob(arg1, options = {}) {
    if (!arg1) {
        return FALSE;
    }
    // Glob with String
    ...
    const resultExpression = function (path, basename, hasSibling) {
        ...
        for (let i = 0, n = parsedPatterns.length; i < n; i++) {
            ...
            if (parsedPattern.requiresSiblings && hasSibling) {
                ...
                if (!name) {
                    name = basename.substr(0, basename.length - (0, path_1.extname)(path).length);
                }
            }
    ...
    if (value) {
        ...
        if (typeof when === 'string') {
            const result = (path, basename, name, hasSibling) => {
                ...
                return matched instanceof Promise ? matched.then((m) => (m ? pattern : null)) : matched ? pattern : null;
            };
