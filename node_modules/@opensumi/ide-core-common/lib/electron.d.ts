...
export interface IElectronMainApi<Events> {
    on(event: Events, listener: (...args: any[]) => void): IDisposable;
}
...
export interface IElectronMainUIServiceShape {
     ...
     */
    setZoomFactor(webContentsId: number, options: {
        value?: number;
        delta?: number;
...
export declare const IElectronMainUIService = "IElectronMainUIService";
export interface IElectronMainLifeCycleService extends IElectronMainApi<void> {
    minimizeWindow(windowId: number): any;
    fullscreenWindow(windowId: number): any;
    maximizeWindow(windowId: number): any;
    unmaximizeWindow(windowId: number): any;
    closeWindow(windowId: number): any;
    reloadWindow(windowId: number): any;
    /**
     * 在某个窗口打开新的工作区
     * @param workspace 工作区主Uri
     * @param options
     */
    openWorkspace(workspace: string, options?: any): any;
    setExtensionDir(path: string, windowId: number): any;
    setExtensionCandidate(candidate: ExtensionCandidate[], windowId: number): void;
}
export declare const IElectronMainLifeCycleService = "IElectronMainLifeCycleService";
export interface IURLHandler {
    handleURL(url: string): Promise<boolean>;
}
export declare const IElectronURLService = "IElectronURLService";
